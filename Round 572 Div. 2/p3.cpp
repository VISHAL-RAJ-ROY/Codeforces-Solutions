...
int query_non_idem( ve<vii> & sptable, int l, int r ) {
...
ii func( int a, int b ) {
    return { (a+b >= 10 ? 1 : 0 ), (a+b)%10 };
}

ii func( ii a, ii b ) {
    return { a.xx+b.xx+(a.yy+b.yy >= 10 ? 1 : 0 ), (a.yy+b.yy)%10 };
}

ve<vii> store_in_spare_table( vi & v ) {
    int N = sz(v), K = log2(N)+1; 
    ve<vii> sptable(N,vii(K,{0,0}));
    for( int i = 0 ; i < N-1; i++ ) sptable[i][1] = func(v[i],v[i+1]);
    FOR(j,2,K)
        FOR(i,0,N-(1<<j)+1)
...
int main() {
    ...
    int q; cin>>q;
    ve<vii> sp = store_in_spare_table(v);
    FOR(i,0,q) {
        int x, y; cin>>x>>y;
        x--;y--;
        cout<<query_non_idem(sp,x,y)<<"\n";
    }
    return 0;
}
