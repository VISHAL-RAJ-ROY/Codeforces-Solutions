...
** Problem Link -> https://codeforces.com/problemset/problem/660/F
*/
#include <bits/stdc++.h>
...
const li oo = 1e14; // don't use for long long
...
int main() {
    ...
    li ma = 0;
    // read("rr.txt");
    int n; cin>>n; vl v(n+1,0), s(n+1,0), z(n+1,0); FOR(i,1,n+1) cin>>v[i], s[i] = v[i]+s[i-1], z[i] = z[i-1]+i*v[i];
    FOR(i,n+1,1) ma = max(ma,z[n]-(i-1)*s[n]+(i-1)*s[i-1]-z[i-1]);
    LineContainer lon;
    FOR(i,n,1) {
        lon.add(-1*s[i],z[i]);
        li curr = lon.query(i-1);
        ma = max(ma,(i-1)*s[i-1]-z[i-1]+curr);
    }
    cout<<ma<<"\n";
    return 0;
}
